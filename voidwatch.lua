...
local function buy_cell()
	local p = packets.new('outgoing', 0x5b, {
			...
			['_unknown1'] = tonumber(cell_unknown),
		})
	packets.inject(p)
end
	...
	if id == 0x34 then
		if wait_for_officer_0x34 then
			...
			if voidwatch_officers[zone] then
				-- 0x41 = 1 cobalt 0100 0001
				-- 0x81 = 2 cobalt 1000 0001
				-- 0xC118 = 99 cobalt 0001 1000 1100 0001
				-- 0x42 = 1 rubicund 0100 0010
				-- 0x0201 = 4 rubi 0001 0000 0010
				-- last 6 bits is 1 for cobalt and 2 for rubicund
				-- other bits are the number of cells to buy
				-- so just need to bitshift the buy number to left by 6 and add 1 or 2 for cobalt or rubicund
